...
class External(QtCore.QObject):
  ...
  def arbiterInformSerialized(self, eventname, payload):
    ...
    # contained surrogate pairs (unicode chars that don't fit in a wchar, like
    # these: "𝟘𝟙𝟚𝟛𝟜𝟝𝟞𝟟𝟠𝟡"), those pairs would be parsed correctly into single
    # Python characters, but an extra '\x00' character was appended to the end
    # of the string for each pair. JSON decoding choked on those, so we removed
    # them here. Keeping this around just in case.
